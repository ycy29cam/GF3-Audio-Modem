...
TX_REPS         = 10             # 1 pilot + 7 identical data blocks (Comment seems outdated, it's TX_REPS pilot/data pairs)
...
def prepare_tx_sequence(plot = False) -> dict:
    ...
    for i in range(200):
        ...
        freq_symbols, _ = qpsk_gray(bits)
        data_freq_symbols_.append(freq_symbols)
        block_no_cp = to_real_ofdm_block(freq_symbols)
    ...
    if plot:
        ...
        plt.plot(waveform_scaled, lw=0.7)
    ...
    info = {
        ...
        "cp_len"                 : CP_LEN,
        "block_real?"            : np.isrealobj(payload.append(time_pilot_blocks_no_CP[2])),
        "total_ofdm_length"      : calculated_total_ofdm_length,
        "final_len"              : len(waveform_scaled),

...
from Transmitter3000NEW import generate_chirp, WAV_TX, Q_COL
import Transmitter3000NEW as tx
import ldpc_jossy
import pickle
    ...
    while current_offset < byte_array.size:
        ...
        try:
            ...
            if content_end_abs > byte_array.size:
                print(
                    f"Error: Required file content (ends at {content_end_abs}) exceeds byte array size ({byte_array.size}). Stopping.")
                break
            ...
            if file_name.endswith(".tiff") or file_name.endswith(".tif"):
                ...
                with open(output_filename, "wb") as f:
                    f.write(file_content.tobytes())
                print(f"SUCCESS: Saved decoded TIFF file as '{output_filename}'")

            elif file_name.endswith(".wav"):
                output_filename = f"Decoded_File_{file_count}.wav"
                with open(output_filename, "wb") as f:
                    f.write(file_content.tobytes())
                print(f"SUCCESS: Saved decoded WAV file as '{output_filename}'")

            elif file_name.endswith(".txt"):
                output_filename = f"Decoded_File_{file_count}.txt"
                with open(output_filename, "wb") as f:
                    f.write(file_content.tobytes())
                print(f"SUCCESS: Saved decoded text file as '{output_filename}'")

            else:
                # Handle unknown file types or simple ASCII text
                output_filename = f"Decoded_File_{file_count}.dat"  # Save as a generic binary file
                with open(output_filename, "wb") as f:
                    f.write(file_content.tobytes())
                print(f"WARNING: Unknown file type for '{file_name}'. Saved as binary '{output_filename}'")
            ...
            # For debugging, show the bytes around the error

...
def prepare_tx_sequence(plot=False) -> dict:
    ...
    for rep in range(TX_REPS):
        raw_bits = random_bitpairs(n_qpsk, seed_no=4242 + rep)
        freq_pilot, col_pilot = qpsk_gray(raw_bits)

        # OFDM‐modulate those QPSK symbols
        pilot_td = to_real_ofdm_block(freq_pilot)
        ...
        pilot_colours.append(col_pilot)
    ...
    np.save(PILOT_TIME_NO_CP_NPY, np.array(time_pilot_blocks_no_cp))
    np.save(PILOT_NPY, np.array(pilot_freq_symbols))
    np.save(COLMAP_NPY, np.stack(pilot_colours))  # same shape, for plotting

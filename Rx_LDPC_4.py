...
from Tx_LPDC_test import generate_chirp, WAV_TX, output, Q_COL
import Tx_LPDC_test as tx
import ldpc_jossy
...
def ldpc_decode_cw(llr_vec: np.ndarray) -> np.ndarray:
    ...
    return (soft < 0).astype(np.uint8)[:LDPC_K]   # information bits only
...
if __name__ == "__main__":
    ...
    # 10)  LDPC decode & BER  ----------
    decoded_info_bits = []  # [(info1 , info2) …] for every OFDM symbol

    #def to_bits(sym):
    #    """QPSK symbol → two hard bits."""
    #    return (sym.real < 0, sym.imag < 0)  # bool → 0 / 1 later

    for blk_idx, blk_syms in enumerate(reconstructed_data):

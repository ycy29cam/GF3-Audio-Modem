...
def OFDM_pilot(length = None):
    ...
    np.save("labels.npy", labels) # Store colour assignment as label.npy, which can be loaded locally

    x = np.array([symbol_map[tuple(b)] for b in B_matrix]) # x is useful data randomly generated by given seed
    np.save("pilot_no_zero.npy", x)
...
# The above command was commented out so the testsignal.wav file isn't overwritten
